...
function providerConfig(env = process.env) {
  const requestedProvider = String(env.LLM_PROVIDER || 'openai').toLowerCase();
  if (requestedProvider !== 'openai') {
    const error = new Error('AI WORKS permanently uses the OpenAI API. Alternative providers are not supported in this project.');
    error.code = 'UNSUPPORTED_PROVIDER';
    throw error;
  }

  const baseUrl = String(env.OPENAI_BASE_URL || env.LLM_BASE_URL || DEFAULT_BASE_URL).replace(/\/$/, '');
  ...
  return {
    ...
    baseUrl,
    model: env.OPENAI_MODEL || env.LLM_MODEL || DEFAULT_MODEL,
  };
}
